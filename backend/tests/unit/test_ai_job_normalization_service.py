...
def service(mock_openai_service):
    ...
    with patch('app.services.ai_job_normalization_service.OpenAIService', return_value=mock_openai_service):
...
class TestAIJobNormalizationService:
    ...
    async def test_normalize_job_title_with_context(self, service, mock_openai_service):
        ...
        # Act
        result = await service.normalize_job_title("PM", "Product", experience_level="mid")
    ...
    @pytest.mark.asyncio
    async def test_normalize_job_title_already_standard(self, service, mock_openai_service):
        ...
        # Act
        result = await service.normalize_job_title("Senior Software Engineer", "Engineering")

        # Assert
        assert result["normalized_title"] == "Senior Software Engineer"
        assert result["confidence"] >= 0.9  # High confidence for already-standard titles

    @pytest.mark.asyncio
    async def test_normalize_job_title_handles_openai_errors(self, service, mock_openai_service):
        ...
        assert "API Error" in str(exc_info.value) or "Failed to normalize" in str(exc_info.value)
    ...
    @pytest.mark.asyncio
    async def test_extract_skills_success(self, service, mock_openai_service, sample_job):
    ...
    @pytest.mark.asyncio
    async def test_extract_skills_from_requirements_only(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_suggest_salary_range_remote_position(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_suggest_salary_range_with_empty_skills(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_suggest_salary_range_invalid_range(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_normalize_job_batch_success(self, service, mock_openai_service, sample_job):
    ...
    @pytest.mark.asyncio
    async def test_normalize_job_batch_with_failures(self, service, mock_openai_service, sample_job):
        ...
        mock_openai_service.generate_completion.side_effect = [
            # Job 1
            {"content": "Senior Software Engineer", "usage": {"prompt_tokens": 30, "completion_tokens": 5, "total_tokens": 35}},
            {"content": '{"skills": ["React"], "confidence": 0.9}', "usage": {"prompt_tokens": 50, "completion_tokens": 20, "total_tokens": 70}},
    ...
    @pytest.mark.asyncio
    async def test_confidence_score_high_for_standard_titles(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_confidence_score_lower_for_ambiguous_titles(self, service, mock_openai_service):
    ...
    @pytest.mark.asyncio
    async def test_complete_normalization_workflow(self, service, mock_openai_service, sample_job):
        ...
        assert enriched_job["normalized_title"]["normalized_title"] == "Senior Software Engineer"

...
def upgrade() -> None:
    # Create email_delivery_logs table (use VARCHAR with check constraint instead of enum to avoid conflicts)
    ...
    op.create_table(
        ...
        sa.Column('blocked_by_user_id', UUID(as_uuid=True), nullable=True),
    ...
    op.create_index('ix_email_unsubscribes_user_id', 'email_unsubscribes', ['user_id'])
...
def downgrade() -> None:
    ...
    op.drop_index('ix_email_unsubscribes_user_id', table_name='email_unsubscribes')

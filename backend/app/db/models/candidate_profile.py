...
class CandidateProfile(Base):
    ...
    availability_status = Column(
        String(50), server_default="not_looking"
...
class CandidateView(Base):
    ...
    )  # Company member who viewed
    candidate_id = Column(
        GUID(), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )  # Candidate being viewed
    ...
    context_job_id = Column(
        GUID(), ForeignKey("jobs.id", ondelete="SET NULL")
    )  # If viewed in context of a job
    ...
    viewer = relationship(
        "User", foreign_keys=[viewer_id], backref="candidate_views_made"
    )
    candidate = relationship(
        "User", foreign_keys=[candidate_id], backref="candidate_views_received"
    )

...
class JobCreate(BaseModel):
    ...
    company_name: str = Field(..., min_length=1, max_length=255, description="Company name display")
    ...
    employment_type: EmploymentType = Field(..., description="Full-time/part-time/contract/internship")

    # Experience
    experience_level: Optional[ExperienceLevel] = Field(None, description="Experience level required")
    experience_min_years: Optional[int] = Field(None, ge=0, le=50, description="Minimum years of experience")
    experience_max_years: Optional[int] = Field(None, ge=0, le=50, description="Maximum years of experience")
    experience_requirement: Optional[str] = Field(None, max_length=100, description="Experience requirement text")
    ...
    benefits: Optional[List[str]] = Field(default_factory=list, description="Company benefits")

    # Additional
    requires_visa_sponsorship: bool = Field(default=False, description="Whether visa sponsorship is available")
    external_url: Optional[str] = Field(None, max_length=500, description="External application URL")
    ...
    def validate_experience_range(cls, v, info):
        ...
        if experience_min is not None and v is not None:
            if v < experience_min:
                raise ValueError('experience_max_years must be greater than or equal to experience_min_years')
        return v

...
class CoverLetterGenerationRequest(BaseModel):
    ...
    resume_version_id: Optional[str] = Field(
        None, description="Specific resume version to use"
    )
    resume_id: Optional[str] = Field(
        None, description="Resume ID (uses default version)"
    )
    job_title: str = Field(
        ..., min_length=1, max_length=255, description="Target job title"
    )
    company_name: str = Field(
        ..., min_length=1, max_length=255, description="Company name"
    )
    job_description: Optional[str] = Field(
        None, max_length=10000, description="Full job description"
    )
    tone: CoverLetterTone = Field(
        default=CoverLetterTone.PROFESSIONAL, description="Desired tone"
    )
    length: CoverLetterLength = Field(
        default=CoverLetterLength.STANDARD, description="Desired length"
    )
    emphasize_skills: List[str] = Field(
        default=[], max_length=10, description="Skills to highlight"
    )
    custom_intro: Optional[str] = Field(
        None, max_length=500, description="Custom introduction paragraph"
    )
    company_research: Optional[str] = Field(
        None, max_length=1000, description="Company insights to incorporate"
    )
    referral_name: Optional[str] = Field(
        None, max_length=255, description="Referral contact name and title"
    )
    address_gap: Optional[str] = Field(
        None, max_length=500, description="Explanation for employment gap"
    )
    career_change_context: Optional[str] = Field(
        None, max_length=500, description="Career transition context"
    )
    strict_factual: bool = Field(default=True, description="Prevent AI hallucinations")
    variations_count: int = Field(
        default=1, ge=1, le=3, description="Number of variations to generate"
    )
...
class CoverLetterEditRequest(BaseModel):
    ...
    content: Optional[str] = Field(
        None, max_length=2000, description="New content for section"
    )
    regenerate: bool = Field(
        default=False, description="Regenerate this section with AI"
    )
    instructions: Optional[str] = Field(
        None, max_length=500, description="Instructions for regeneration"
    )

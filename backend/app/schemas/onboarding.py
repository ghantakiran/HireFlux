...
class JobPreferencesUpdate(BaseModel):
    ...
    target_titles: List[str] = Field(..., min_items=1, max_items=10, description="Target job titles")
    ...
    industries: List[str] = Field(default=[], max_items=10, description="Target industries")
...
class SkillsUpdate(BaseModel):
    ...
    skills: List[SkillInput] = Field(..., min_items=1, max_items=50, description="User skills")
...
class OnboardingProgress(BaseModel):
    ...
    current_step: int = Field(..., ge=1, le=4, description="Current onboarding step (1-4)")

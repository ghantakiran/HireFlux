...
class TopJob(BaseModel):
    ...
    avg_candidate_fit: Optional[float] = Field(
        None, description="Average fit index of applicants"
    )
...
class DashboardStats(BaseModel):
    ...
    avg_time_to_first_application_hours: Optional[float] = Field(
        None, description="Average hours from job posting to first application"
    )
    avg_candidate_quality: Optional[float] = Field(
        None, description="Average fit index of all applicants (0-100)"
    )

    # Usage tracking
    jobs_posted_this_month: int = Field(
        0, description="Jobs posted in current billing month"
    )
    candidate_views_this_month: int = Field(
        0, description="Candidate profiles viewed this month"
    )

    # Plan limits
    max_active_jobs: int = Field(
        ..., description="Maximum active jobs allowed on current plan"
    )
    max_candidate_views: int = Field(
        ..., description="Maximum candidate views allowed per month"
    )
...
class ActivityEvent(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Type of event: job_posted, application_received, etc."
    )
    title: str = Field(..., description="Human-readable event title")
    description: Optional[str] = Field(None, description="Event description")
    actor_name: Optional[str] = Field(
        None, description="Name of team member who triggered event"
    )
    job_title: Optional[str] = Field(
        None, description="Related job title if applicable"
    )
    candidate_name: Optional[str] = Field(
        None, description="Related candidate name if applicable"
    )
...
class RecentActivity(BaseModel):
    ...
    events: List[ActivityEvent] = Field(
        default_factory=list, description="Recent activity events (last 30 days)"
    )
...
class TeamActivity(BaseModel):
    ...
    member_activities: List[TeamMemberActivity] = Field(
        default_factory=list, description="Activity breakdown by team member"
    )

...
class PortfolioItemBase(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=500, description="Brief description"
    )
...
class CandidateProfileBase(BaseModel):
    ...
    headline: Optional[str] = Field(
        None, max_length=255, description="Professional headline"
    )
    ...
    preferred_roles: Optional[List[str]] = Field(
        None, description="Preferred job roles"
    )
    preferred_location_type: Optional[str] = Field(
        None, description="Remote/hybrid/onsite preference"
    )
    ...
    expected_salary_min: Optional[Decimal] = Field(
        None, description="Minimum expected salary"
    )
    expected_salary_max: Optional[Decimal] = Field(
        None, description="Maximum expected salary"
    )
    expected_salary_currency: Optional[str] = Field(
        "USD", max_length=3, description="Currency code"
    )

    # Resume
    resume_summary: Optional[str] = Field(None, description="Brief resume summary")
    latest_resume_url: Optional[str] = Field(
        None, max_length=500, description="URL to latest resume"
    )

    # Visibility
    visibility: str = Field(
        "private", description="Profile visibility: public or private"
    )
...
class CandidateProfileCreate(CandidateProfileBase):
    ...
    availability_status: Optional[str] = Field(
        "not_looking", description="Availability status"
    )
    availability_start_date: Optional[date] = Field(
        None, description="When candidate is available"
    )
...
class CandidateViewCreate(BaseModel):
    ...
    context_job_id: Optional[UUID] = Field(
        None, description="Job context if applicable"
    )
...
class CandidateSearchFilters(BaseModel):
    ...
    max_years_experience: Optional[int] = Field(
        None, le=50, description="Maximum years"
    )
    ...
    availability_status: Optional[List[str]] = Field(
        None, description="Availability statuses"
    )
